    ...
    for (var i = 0; i < additionalQuery.docs.length; i++) {
      ...
      if (additionalDoc['customer_config'] == "edition") {
        ...
        switch (additionalDoc['type']) {
          ...
          case "radio-button":
            QuerySnapshot radiobuttonResponseQuery =
                await additionalDoc.reference.collection('radiobutton').get();

            int index = 0;
            for (var i = 0; i < radiobuttonResponseQuery.docs.length; i++) {
              DocumentSnapshot radiobuttonDoc =
                  radiobuttonResponseQuery.docs[i];
              if (radiobuttonDoc['label'] ==
                  additionalResponseDoc['response_label']) {
