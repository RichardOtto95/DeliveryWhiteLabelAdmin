...
import 'package:delivery_admin_white_label/app/core/models/seller_model.dart';
import 'package:delivery_admin_white_label/app/core/services/auth/auth_service_interface.dart';
...
class AuthService implements AuthServiceInterface {
  ...
  Future preRegisteredUser(seller) async {
    ...
    try {
      ...
      print('preRegisterUserQuery: ${preRegisterUserQuery.docs.length}');
  ...
  Future<Seller?> handleSignup(seller) async {
    ...
    if (seller != null) {
      ...
      DocumentReference additionalObservationRef =
          await sellerRef.collection('additional').add({
      ...
      additionalObservationRef.update({"id": additionalObservationRef.id});

      DocumentReference additionalAmountRef =
          await sellerRef.collection('additional').add({
        "created_at": FieldValue.serverTimestamp(),
        "hint": null,
        "id": null,
      ...
      additionalAmountRef.update({"id": additionalAmountRef.id});

      DocumentReference additionalNewOrOld =
          await sellerRef.collection('additional').add({
        "created_at": FieldValue.serverTimestamp(),
        "hint": null,
        "id": null,
        ...
        "index": 0,
        "auto_selected": true,
        "seller_config": "edition",
        "customer_config": "reading",
        "alignment": "left",
        "font_size": 18,
      });
      await additionalNewOrOld.update({"id": additionalNewOrOld.id});

      for (var i = 0; i < 1; i++) {
        String label = i == 0 ? "Novo" : "Usado";
        DocumentReference ref =
            await additionalNewOrOld.collection('radiobutton').add({
          "id": "",
          "index": i,
          "label": label,
          "value": 0,
